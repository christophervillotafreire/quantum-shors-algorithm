import argparse # Librería estándar para interceptar flags/argumentos pasados desde la terminal bash
import configparser # Librería estándar para leer archivos locales de ejecución masiva (.ini)
import sys
from typing import Literal, Union
from common.settings import get_settings_value_for_key # Interfase con nuestro settings.yaml maestro
...
class CommandLineParser:
    """
    Controlador y traductor de todos los parámetros de entrada del usuario.
    Permite construir diferentes experimentos a medida desde la línea de comandos,
    pudiendo prender/apagar SABRE, mitigación de errores o tipos de simulador.
    """

    def __init__(self):
        # Inicializamos el parseador que generará al usuario la típica pantalla de ayuda `--help`
    ...
    def int_or_auto(value: str) -> Union[int, Literal['auto']]:
        """Pequeño tipador auxiliar para aceptar números enteros estrictos O la cadena textual 'auto'."""
        if value == 'auto':
            return 'auto'
        try:
            return int(value)
    ...
    def add_arguments(self):
        """
        Define uno a uno cada modificador/atributo disponible para ejecución (como --number-to-factor o -n).
        Asocia también las listas de valores "por defecto" extraídas del archivo YAML para sugerencias en caso
        de que el usuario olvide proveer el argumento.
        """

        # Get list of configurable values from settings.yaml file
        # Recolectamos opciones pre-diseñadas para limitar con `choices=` a los usuarios en terminal
    ...
    def parse_arguments(self):
        """Dispara y evalúa todo lo que escribió el usuario en consola; atrapando las variables resultantes"""
        self.args = self.parser.parse_args()
        return self.args

    def set_defaults(self, defaults):
        """Permite inyectar por debajo una base de valores si el usuario no los proveyó o provienen de un archivo .INI"""
        self.parser.set_defaults(**defaults)


class ConfigurationIni:
    """
    Controlador para lecturas de archivos de texto .ini. 
    Ideal cuando lanzar ensayos masivos (`bash`) con 50 modificadores (args) uno a uno en terminal es molesto e ilegible. 
    Se empaquetan en un archivo y el programa los consume simulando que la persona los tecleó.
    """
    ...
    @staticmethod
    def check():
        """Escanea superficialmente entre lo tecleado para ver si el usuario invocó la bandera rápida `--config`"""
        check = False
    ...
    def load(self, config_file):
        """Parsea el archivo INI en memoria RAM listando todas sus secciones como arreglos o diccionarios"""
        self.config_parser.read(config_file)

    def create_defaults(self):
        """
        Recorre las secciones conocidas del archivo de automatización ('transpiler', 'sampler', etc.), 
        transforma sus valores leídos como simples textos ('true/false/vacio') en su equivalente lógico de python 
        y los retorna para que puedan ser aplicados encima del ArgumentParser.
        """
        defaults = {}
        ...
        for key, value in defaults.items():
            if value in ['true', 'false']:
                # Convierte a booleanos nativos de Python
                defaults[key] = value.lower() == 'true'
            if key == "random_a":
                # Si viene una lista por comas separadas (13, 2, 8), parsearla a Arreglo [13, 2, 8] evadiendo los que sean 0.
...
class JobIdArgsValidator:
    ...
    def validate_ibm_account_name(self):
        # Medida de validación estándar preventiva para verificar que una llamada a Nube no carezca de canal.
